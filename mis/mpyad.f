...
C
   50 MPY(1)= NAME(1)
      MPY(3)= JBEGN
      LMCH  = MACH.EQ.2 .OR. MACH.GE.5
      NOUT  = MOUT
      IF (LMCH .AND. L19.EQ.0) NOUT = 4
C
...
C
C
C     TURN TRANSPOSE FLAG OFF IF INPUT MATRIX A IS SYMMETRIC, AND MAKE
C     SURE THAT COLUMNS EQUEL ROWS, AND DIAG 41 IS OFF.
C
...
C     MPY-D-R-I. IN LOGFILE, A DOT IS ADDED IN  'MPYAD.  BEGN'
C
      K = FILEA(4)
...
C        DIAGONAL    ROW VECTOR    IDENTITY
C
      IF (TYPEA.NE.TYPEB .OR. TYPEA.NE.TYPD) GO TO 80
      NX1 = M
      IF (K.EQ.3 .OR. K.EQ.7) NX1 = N
      IF (T.EQ.1 .AND. NX1.NE.1) GO TO 80
      K = MAX0(M,N,Q,R)
      J = K*2 + 1
      K = K + 1
      MPY(2) = NAME3
...
C
   80 MPY(2) = NAME(2)
     ...
     1    PRC(TYPEB).EQ.2)) TYPED = RCD + 1
      ...
      WRITE  (MOUT,90) UFM
   90 FORMAT (A23,' 2398, MPYAD REQUIRES SIGN OF A*B TO BE -1,0, OR +1')
      GO TO 540
  100 CALL MPYQ (Z)
C
...
C
      ...
      IF (L19 .EQ. 0) GO TO 180
C     IF (LMCH) GO TO 150
      CALL PAGE2 (4)
      WRITE  (NOUT,140) UIM
  140 FORMAT (A29,' 2432, DIAG 19 MPYAD SUMMARY')
      ICRG = IBLNK
C     GO TO 160
C 150 ICRG = IZERO
C 160 CONTINUE
      WRITE (NOUT,170,ERR=180) ICRG,FILEA(1),N,M,IELEMS,RHOA,
     1                              FILEB(1),R,Q,JELEMS,RHOB,
     2                              T,CORE,MPASS1,MPASS2,
     3                              MPASS3,TIME1,TIME2,TIME3
  170 FORMAT (A1,49H(A MATRIX ROWS  COLS   TERMS    DENS)*(B MATRIX R
     1       ,50HOWS  COLS   TERMS    DENS) T   CORE   P1  P2  P3
     2       ,30H   TIME1     TIME2     TIME3   /
     3        I7, I8, I6, I9, F7.4,
     4        I8, I8, I6, I9, F7.4,
     5        I3, F8.0, 3I4, 2E10.3, 1X, E9.2)
C
...
C
      ...
      GO TO 600
  190 IF (TIME1.LT.TIME2 .AND. TIME1.LT.TIME3) GO TO 200
...
C
  200 JCOL = 1
      ...
      CALL PAGE2 (2)
      WRITE  (NOUT,210) UIM
  210 FORMAT (A29,' 2433, MPYAD')
      ICRG = IPLUS
      GO TO 230
  220 ICRG = IBLNK
  230 WRITE  (NOUT,240) ICRG,METHOD(T+1),MPASS1,TIME1
  240 FORMAT (A1,49X,'METHOD ',A4,', NBR PASSES =',I4,
     1        ',  EST. TIME =',F9.1)
...
C
C *** FOR VAX AND UNIX MACHINES, MPY1 CALL REQUIRES ARGUMENTS. OTHER
C     MACHINES DO NOT.
C     (RENAME  MPY1 TO MPY1V FOR VAX AND UNIX)
C
      IF (MACH .GE. 5) CALL MPY1V (ZZ,Z,ZD)
      IF (MACH .LT. 5) CALL MPY1
C
...
C
C     IF END RESULT IS A SYMMETRIC MATRIX IN FILED, MAKE SURE IT'S FORM
C     IS 6 (SYMMETRIC). IT WILL SAVE CPU TIME LATER. RARELY, IT COULD
C     BE FORM 8 (IDENTICAL)
C
  380 K = 0
      DO 385 I = 1,21,7
      IF (FILEA(I) .LE. 0) GO TO 385
      J = FILEA(I+3)
      IF (I.EQ.15 .AND. J.EQ.3) GO TO 385
C            FILEC      DIAGONAL
C
      IF (J.NE. 6 .AND. J.NE.8) GO TO 390
C             SYMM        IDENT
C
      IF (J .EQ. 6) K = K + 10
      IF (J .EQ. 8) K = K + 1
...
C
C     ------------------------------------------------------
C     -   UNIVAC ONLY - METHOD 2NT DOES NOT ALWAYS WORK,   -
C     -                 SWITCH TO METHOD 1NT.              -
C     -                 BY G.CHAN/UNISYS,  10/1987         -
  600     IF (MACH.EQ.3 .AND. T.EQ.0) GO TO 200
C     ------------------------------------------------------
C
      IF (ROWA.GT.65535 .AND. NBPW.LE.32) WRITE (MOUT,610) M,ROWA
  610 FORMAT ('0*** SYSTEM POTENTIAL ERROR MESSAGE, CURRENTLY THE MUL',
     1        'TIPLY-ADD ROUTINE, MPYAD, IS LIMITED TO 65535 ROWS FOR',
     2        /5X,'THE INPUT MATRIX A, WHICH IS',I9,2H X,I7)
C
...
C
  ...
  620 DFILE = FILED(1)
      ...
      GO TO 640
  630 ICRG = IBLNK
  640 WRITE (NOUT,240) ICRG,METHOD(T+3+MT4),MPASS2,TIME2
C
C     BEGIN PASS
C
...
C
  ...
  650 FILE = DFILE
      ...
      L    = FIRSTL
      ACOL = ACOL1
  660 IF (APOINT .GE. L-2) GO TO 750
      ZZ(L  ) = 0
      ZZ(L-1) = 0
      BLOCK(8)=-1
...
C
  ...
  690 IF (PRC(TYPE) .EQ. 2) POINT = POINT*2 - 1
      ...
      POINT = POINT + INCRA
  700 CONTINUE
  710 ZZ(APOINT) = ORF(ROW,LSHIFT(NBRSTR,IHALF))
C
C     COMMENT FROM G.CHAN/UNISYS   1/92
C     THE ABOVE ZZ(APOINT) LIMITS THE HIGHEST ROW NUMBER TO 65565
C     THUS THIS MPYAD SUBROUTINE CAN NOT HANDLE MATRIX SIZE GREATER THAN
C     65565.   *** WILL FIX THIS LIMITATION NEXT YEAR! ***
C
      ZZ(L-1) = ZZ(L-1) + 1
      APOINT  = KRN + 1
C
...
C
C *** FOR VAX AND UNIX MACHINES, MPY2NT CALL REQUIRES ARGUMENTS. OTHER
C     MACHINES DO NOT.
C     (RENAME  MPY2NT TO MPY2NV FOR VAX AND UNIX)
C
      IF (MACH .GE. 5) CALL MPY2NV (ZZ,Z,ZD)
      IF (MACH .LT. 5) CALL MPY2NT
      GO TO 860
C
...
C
C *** FOR VAX AND UNIX MACHINES, MPY2T CALL REQUIRES ARGUMENTS. OTHER
C     MACHINES DO NOT.
C
  960 IF (MACH .GE. 5) CALL MPY2TV (ZZ,Z,ZD)
      IF (MACH .LT. 5) CALL MPY2T
...
C
 1100 TIME = 0.0
      ...
      CALL PAGE2 (2)
      WRITE  (NOUT,1110) UIM
 1110 FORMAT (A29,' 2434')
      ICRG = IPLUS
      GO TO 1130
 1120 ICRG = IBLNK
 1130 WRITE  (NOUT,1140) ICRG
 1140 FORMAT (A1,49X,'FROM MPYAD - NULL MATRIX PRODUCT')
...
C     ==============================
C
 1300 IF (LMCH) GO TO 1310
      CALL PAGE2 (2)
      WRITE (NOUT,210) UIM
      ICRG = IPLUS
      GO TO 1320
 1310 ICRG = IBLNK
 1320 WRITE (NOUT,240) ICRG,METHOD(5),MPASS3,TIME3
      BLOCK(1) = FILEB(1)
      ACORE = ORF(ND+1,1)
      CFILE = SCRTCH
      DFILE = FILED(1)
