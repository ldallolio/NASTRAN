...
CZZ   COMMON /ZZIFPX/ IBUFF(1)
     ...
     2                IDFREQ,IDRAD,NVAR,IDS,JMS,KMS,LPLF
      ...
      DATA ERIGID/4H1   /, FRIGID/4H2   /, GRIGID/4H3   /, IND /4HIN  /
...
C
C     REMOVE THE CHANGES DONE IN 92 AND BRING BACK THE 91 CODE
C
...
C
C6510 IM = IM + 1
C     IF (MF(1) .EQ. 3) GO TO 6560
C     IF (IM    .GE. 4) GO TO 6565
C     L1 = 1
C6520 DO 6540 L = L1,8
C     IF (MF(L) .NE. 2) GO TO 6530
C
C ... REAL NUMBERS
C
C     IF (IM-2) 6521,6522,6523
C6521 IF (L .NE. 5) BADDAT = .TRUE.
C     GO TO 6525
C6522 IF (L.NE.2 .AND. L.NE.7) BADDAT = .TRUE.
C     GO TO 6524
C6523 IF (L .NE. 4) BADDAT = .TRUE.
C6524 N   = N + 1
C     I(N)= -1
C6525 N   = N + 1
C     I(N)= M(L)
C     GO TO 6540
C
C ... INTEGERS OR BLANKS
C
C6530 IF (MF(L) .EQ. 0) GO TO 6540
C     IF (MF(L).NE.1 .OR. M(L).LE.0) BADDAT = .TRUE.
C     IF (IM-2) 6531,6532,6533
C6531 IF (L .EQ. 5) BADDAT = .TRUE.
C     IF (L-6) 6535,6534,6535
C6532 IF (L.EQ.2 .OR. L.EQ.7) BADDAT = .TRUE.
C     IF (L.EQ.3 .OR. L.EQ.8) GO TO 6534
C     GO TO 6535
C6533 IF (L .EQ. 4) BADDAT = .TRUE.
C     IF (L-5) 6535,6534,6535
C6534 IF (IFPDCO(M(L))) BADDAT = .TRUE.
C6535 N  = N + 1
C     I(N) = M(L)
C6540 CONTINUE
C     IF (M1(1) .NE. 0) GO TO 6550
C6545 KN = 1
C     GO TO 9
C6550 N  = N + 1
C     I(N) = -1
...
C     GO TO 6570
C6565 IM = IM + 1
C     IF (IM    .GT. 5) BADFOR = .TRUE.
C     IF (MF(1) .NE. 0) BADFOR = .TRUE.
C     L1 = 2
C6570 DO 6580 L = 2,6,2
C     IF (MF(L) .EQ. 0) GO TO 6575
C     IF (MF(L  ).NE.1 .OR. M(L1  ).LE.0) BADDAT = .TRUE.
C     IF (MF(L+1).NE.1 .OR. M(L1+1).LE.0) BADDAT = .TRUE.
C     IF (IFPDCO(M(L1+1))) BADDAT = .TRUE.
C     I(N+1) = M(L1  )
C     I(N+2) = M(L1+1)
C     N  = N  + 2
C6575 L1 = L1 + 2
C6580 CONTINUE
C     IF (MF(8) .NE. 0) BADFOR = .TRUE.
C     IF (M1(1) .NE. 0) GO TO 6555
C     GO TO 9
C
C --- END 92 CODE:
C
