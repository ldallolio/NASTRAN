...
C
  ...
  100 T  = E(L) - E(L1)
      ...
      E(L ) = E1
      E(L1) = E2
C     WRITE (IO,135) L ,E(L )
C     WRITE (IO,135) L1,E(L1)
      W(L1) = 0.D0
      GO TO 70
  110 SHIFT = E1
...
C     WRITE (IO,135) K,E(K)
  130 IF (DABS(GG) .LT. DELTA) GG = GG + C*DELTA*DSIGN(1.D0,GG)
      ...
      E(K) = GG + SHIFT
C     WRITE  (IO,135) K,E(K)
C 135 FORMAT (11H VALUE SNAP,I5,D16.8)
      GO TO 70
  140 DO 150 I = 1,M
  150 E(I) = E(I)/SCALE
      DO 155 L = 1,M1
...
C     CMAX= 0.D0
      DO 380 I = 1,M
      W(I) = W(I)/S
C     IF (NITER .NE. 1) GO TO 380
C     IF (DABS(W(I)) .GT. DABS(W(ISI))) ISI = I
  380 CONTINUE
C     CMAX = DABS(W(ISI))
C     RATV = DABS(PMAX/CMAX-1.D0)
C     PMAX = CMAX
