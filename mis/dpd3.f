...
C     IF (MACH.EQ.5 .OR.  MACH.EQ.21) CALL SORTCM (0,0,1,1,Z(J+1),N)
C     IF (MACH.NE.5 .AND. MACH.NE.21) CALL SORT   (0,0,1,1,Z(J+1),N)
...
C     IF (MACH .EQ. 5) CALL SORTCM (0,0,1,1,Z(J+1),N)
C     IF (MACH .NE. 5) CALL SORT   (0,0,1,1,Z(J+1),N)
      CALL SORTF (0,0,1,1,Z(J+1),N)
      J1 = J + 2
      JN = J + N
      IX = J + 1
