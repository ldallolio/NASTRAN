...
C
      ...
      I6 = I5 + N2
C
C     CHECK CORE SIZE FOR WILVEC
C
      ICRQ = I6 + 2*ISYS - KORSZ(A)
      IF (ICRQ .GT. 0) CALL MESAGE (-8,ICRQ,WIL)
C
C     TRIDIAGONALIZATION.
C
      IF (N .GT. 2) GO TO 101
      CALL SMLEIG (A(ID),A(IO),A(IV))
      IF (N-2) 300,200,300
  101 TRI(3) = IBEGN
      CALL CONMSG (TRI,3,0)
