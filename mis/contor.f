...
C
  210 RCOLOR = ICOLOR
      RCNTRL = NCNTR
      DO 230 IK = 1,NCNTR
C     IF (NCNTR .LE. ICOLOR) PEN = 31 + IK*ICOLOR/NCNTR
C     IF (NCNTR .GT. ICOLOR) PEN = 32 + (IK/(NCNTR/ICOLOR))
C     IF (PEN .GT. (ICOLOR+31)) PEN = 31 + ICOLOR
...
C
      DO 410 I = 1,NCNTR
C     IF (COLOR .EQ. 0) GO TO 355
C     IF (NCNTR .LE. ICOLOR) PEN = I*ICOLOR/NCNTR
C     IF (NCNTR .GT. ICOLOR) PEN = 1 + (I/(NCNTR/ICOLOR))
C     IF (PEN   .GT. ICOLOR) PEN = ICOLOR
C 355 CONTINUE
