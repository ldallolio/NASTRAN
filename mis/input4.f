...
C
      ...
      LOGICAL          BO,SP,CP,DP,MS,TAPEUP,TAPBIT,DEBUG
      INTEGER          OUTFIL(5),TRL(7),NAME(2),SUBNAM(2),INP(14),IZ(1),
...
CZZ  4       /ZZINP4/  CORE(1)
     4       /ZZZZZZ/  CORE(1)
      ...
      DATA    INP   /  4HUT1 ,4HUT2 ,4H****,4HINPT,4HINP1,4HINP2,4HINP3,
...
C
      FM = UNF
      IF (BO) FM = FMD
      WRITE  (NOUT,10) UIM,UNITX,INP(UNITX-10),FM
C    1,                BCDOPT,P1,P2,P3,P4
   10 FORMAT (A29,'. INPUTT4 MODULE OPENING FORTRAN TAPE',I4,' (',A4,
     1       ') FOR ',A11,' READ.')
C    2,      /5X,'BCDOPT,P1,P2,P3,P4 =',3I3,1X,2A4,I4)
C
      IF (MACH .GE. 5) GO TO 50
      OPEN (UNIT=UNITX,ACCESS='SEQUENTIAL',STATUS='OLD',FORM=FM,ERR=920)
      FILE   = INP(UNITX-10)
      TAPEUP = TAPBIT(FILE)
      IF (TAPEUP) GO TO 50
      WRITE  (NOUT,20) UFM,FILE,UNITX
   20 FORMAT (A23,'. ',A4,' (TAPE UNIT',I4,') NOT ASSIGNED')
      GO TO 990
C
...
C
  ...
  250 FLAG = 0
      ...
      GO TO 280
C
  260 WRITE  (NOUT,270) UFM,NAME
  270 FORMAT (A23,'. CANNOT OPEN OUTPUT FILE - ',A4)
      GO TO 990
C
...
C
  760 CALL CLOSE  (OUTPUT,1)
      CALL WRTTRL (TRL)
      IF (DEBUG) WRITE (NOUT,770) UIM,NAME,INP(UNITX-10),TRL
  770 FORMAT (A29,' FROM INPUTT4 MODULE. ',2A4,' WAS RECOVERED FROM ',
     1       A4,' INPUT TAPE SUCCESSFULLY.', /5X,'TRAIL =',6I6,I9)
      GO TO 800
C
C     BAD DATA ON INPUT TAPE
C
  780 WRITE  (NOUT,790) UFM,FILE,UNITX,NN,IMHERE
  790 FORMAT (A23,'. BAD DATA ENCOUNTERED WHILE READING INPUT TAPE ',A4,
...
C     ERRORS
C
  900 WRITE  (NOUT,910) UFM,INP(UNITX-10),BO,NCOL,NROW,NFORM,NTYPE,NAME,
     1        BCDOPT
  ...
  930 FORMAT (A23,'. INPUTT4 MODULE CANNOT OPEN FORTRAN INPUT TAPE',I4)
      GO TO  990
  940 WRITE  (NOUT,950) UFM,FILE,UNITX,NN,IMHERE
  950 FORMAT (A23,' 3001, EOF ENCOUNTERED WHILE READING INPUT TAPE ',A4,
     1       ' FORTRAN UNIT',I4,',  DATA BLOCK',I4, /5X,'IMHERE =',I4)
      IF (IMHERE.EQ.210 .OR. IMHERE.EQ.220) WRITE (NOUT,975)
      GO TO 990
  960 WRITE  (NOUT,970) UFM,FILE,UNITX,NN,IMHERE
  970 FORMAT (A23,'. BAD DATA IN HEADER RECORD ON INPUT TAPE ',A4,
     1       ' FORTRAN UNIT',I4,',  DATA BLOCK',I4, /5X,'IMHERE =',I5)
