...
100   CONTINUE
      ...
      XX(NSTNS+1)=XMIN
      XX(NSTNS+2)=XSCALE
C     CALL PLOT(1.0,-12.0,-3)
C     CALL PLOT(0.0,1.0,-3)
      IF(NPLOT.EQ.2)GO TO 134
C     CALL AXIS(0.0,0.0,16HAXIAL COORDINATE,-16,DELX,0.0,XMIN,XSCALE)
C     CALL AXIS(0.0,0.0,15HSTATIC PRESSURE,15,9.01,90.0,PLOW,PSCALE)
      PSTAT(NSTNS+1)=PLOW
...
120   XX(I)=X(J,I)
C     CALL LINE(XX,PSTAT,NSTNS,1,1,K)
      IF(J.EQ.NSTRMS)GO TO 130
      K=K+1
      IF(J.EQ.IMID)J=NSTRMS
      IF(J.EQ.1)J=IMID
      GO TO 110
C130  CALL PLOT(DELX+2.0,-11.0,-3)
130   CONTINUE
C     CALL PLOT(0.0,1.0,-3)
      IF(NPLOT.EQ.1)GO TO 180
C134  CALL AXIS(0.0,0.0,16HAXIAL COORDINATE,-16,DELX,0.0,XMIN,XSCALE)
134   CONTINUE
C     CALL AXIS(0.0,0.0,6HRADIUS,6,9.01,90.0,RLOW,XSCALE)
      PSTAT(NSTNS+1)=RLOW
      ...
      XX(I)=X(J,I)
140   PSTAT(I)=R(J,I)
C150  CALL LINE(XX,PSTAT,NSTNS,1,0,0)
150   CONTINUE
      PSTAT(NSTRMS+1)=RLOW
      ...
      PSTAT(J)=R(J,I)
160   XX(J)=X(J,I)
C170  CALL LINE(XX,PSTAT,NSTRMS,1,0,0)
170   CONTINUE
C     CALL PLOT(DELX+2.0,0.0,-3)
180   RETURN
      END
