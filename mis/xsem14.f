...
C        
     ...
     1                SCRTCH(3),WORDB(4),WORDE(2),NUMBR(10)        
      ...
      COMMON /OFP1ID/ IDM(2)        
     1       /SYSTEM/ SYSBUF,NOUT,NOGO,XX(71),DBG75        
...
CZZ  6       /ZZXSEM/ DATABF(1)        
     ...
     4        IBLNK / 4H    /,EXIT  / 4HEXIT/        
      DATA    THISLK/ 14    /,SUBNAM/ 4HXSEM, 2H14  /        
...
C        
  200 MODX = RSHIFT(INOSCR(3),16)        
      IF (ANDF(MXLINK(MODX),LSHIFT(1,THISLK-1)) .NE. 0) GO TO 245       
C        
...
C        
C     NOGO COULD BE SET TO 14 BY OFP1A OR OFP1B        
C        
      IF (NOGO .NE. 14) GO TO 240        
      WRITE  (NOUT,230)        
  230 FORMAT ('0*** PREVIOUS ERROR(S) IN OFP1A OR 1B')        
      CALL MESAGE (-37,0,SUBNAM)        
C        
  240 CALL ENDSYS (NAME,PROGMI(14))        
      RETURN        
C        
...
C        
 ...
 1000 IF (MODX .GT. 12) GO TO 1010        
      I = MODX - 2        
      IF (I) 940,940,995        
 1005 GO TO (2069,2070), I        
 1010 IF (MODX .GT. 70) GO TO 1020        
      I = MODX - 68        
      IF (I) 940,940,1005        
 1020 IF (MODX - 81) 940,2081,1030        
 1025 GO TO (2085,2086,2087,2088,2089,2090,2091,2092), I        
 1030 IF (MODX .GT. 92) GO TO 1040        
      I = MODX - 84        
      IF (I) 940,940,1025        
 1040 IF (MODX - 108) 940,2108,1050        
 1050 IF (MODX - 120) 940,2120,1060        
 1060 IF (MODX - 124) 940,2124,1070        
 1065 GO TO (2139,940,2141,2142), I        
 1070 IF (MODX .GT. 142) GO TO 1080        
      I = MODX - 138        
      IF (I) 940,940,1065        
 1080 IF (MODX .EQ. 155) GO TO 2155        
      IF (MODX .GT. 216) GO TO 2000        
      I = MODX - 200        
      GO TO (2201,940, 940,2204, 940,940,940,940,2209,940,940,2212,940, 
     1       2214,2215,2216), I        
 2000 IF (MODX-LXLINK) 940,940,970        
 2003 CALL XCHK        
      GO TO 10        
 2005 CALL XCEI        
      GO TO 10        
