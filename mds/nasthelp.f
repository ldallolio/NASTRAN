...
C
C     SOME OF THE ABOVE COMMENTS MAY NO LONGER TRUE
C
...
C
C     WRITTEN BY G.CHAN/UNISYS            3/1992
...
C     INCOMING SYSBOL, NO. OF BYTE, AND OUTGOING CORRESPONDING SYMBOL
C
C        1   1 124
C        2   1 124
C        :   :  :
...
C
  ...
  215 FORMAT (/,' If your screen loses character in column 1, it is ',
...
C
  240 WRITE  (OUT,250)
  250 FORMAT (//,' Enter NASTRAN User''S Manual selection -', /7X,
...
C
  ...
  440 FORMAT (/,' Enter MESSAGE NUMBER, STOP,',A35)
      READ   (IN,450) A4
  450 FORMAT (A4)
  ...
  510 FORMAT (' *** No Such MESSAGE NO. ***',/)
      GO TO 430
C
  520 WRITE  (OUT,530) FNAME
  530 FORMAT (//1X,A8,' file DOES NOT EXIST')
...
C
  ...
  550 FORMAT (/,' Enter DICTIONARY word, STOP,',A35)
      READ   (IN,560) A6
  560 FORMAT (A6)
      IF (A11 .EQ. UP1) GO TO 300
      DO 570 J = 1,6
...
C     SECTION AND SUBSECTION MUST PRECEEDED BY A BLANK LINE
C
...
C
  ...
  660 FORMAT (/,' Enter KEY WORD, STOP, next page(P), next section(S),',
     1        /,' next sub-section(B),',A35)
...
C
 ...
 1340 FORMAT (/11X,'Y or c/r  = yes more,    N = no more on this item',
     ...
     o        '(Y,N,STOP,1,2,...,',I2,',-n,P,S,B,^,PRINT,H,or c/r) ')
      GO TO 1370
 1350 WRITE  (OUT,1360)
 1360 FORMAT (' ...more? ')
...
C
 1450 IF (LC .EQ. KU) GO TO 1610
      WRITE  (OUT,1460)
 1460 FORMAT (/,' ...EOF. c/r to continue ')
      GO TO 1500
C
 1470 WRITE  (OUT,1480)
...
C
 2000 WRITE  (OUT,2010)
 2010 FORMAT (/,' Enter OUTPUT FILE Name: ')
      READ   (IN,770) A12
      IF (A4.NE.BNK4 .AND. A4.NE.STP4) GO TO 2030
      WRITE  (OUT,2020)
...
C
 ...
 2130 FORMAT (/,I9,' lines printed')
      GO TO 420
C
 2140 WRITE  (OUT,530) OU12
      GO TO 2000
C
 2200 CLOSE  (LU)
      KOUNT = 0
 2210 WRITE  (OUT,2220)
 2220 FORMAT (//,'  *** JOB DONE ***', //,'  Have a  good day')
      IF (.NOT.POPEN) GO TO 2240
      CLOSE  (OU)
      WRITE  (OUT,2230) OU12
 2230 FORMAT (/,'  *** Don''t forget, you have a print file in ',A12)
      POPEN = .FALSE.
 2240 CONTINUE
      END
